...
export default function App() {
  ...
  // MCQ Instant check
  const checkMultipleChoiceAnswer = async (qId) => {
    const currentAns = sessionAnswers[qId];
    if (currentAns.locked || !currentAns.selectedOption) return;

    const attemptsCount = currentAns.attempts + 1;
    let isCorrect = false;
    let feedbackMsg = "";

    // In production source backend, we can request the backend or check it locally if activeQuiz had correct answer
    // For safety, the correct answers are normally validated during final submission. 
    // Here, for student experience, we mock validation or verify. Let's write a mock client feedback 
    // based on comparing with loaded correct_answer (which was set to "HIDDEN" by secured get endpoint, 
    // but when generating we have it in memory of generated response. In case of API reload, we grade it).
    // Let's assume we allow client check:
    const question = activeQuiz.questions.find((q) => q.id === qId);
    
    // We let the student know correct/incorrect (if generated quiz object has it, otherwise ask server)
    if (question.correct_answer && question.correct_answer !== "HIDDEN") {
      isCorrect = currentAns.selectedOption === question.correct_answer;
      feedbackMsg = isCorrect ? "Chính xác!" : attemptsCount >= 2 ? `Hết lượt! Đáp án đúng là ${question.correct_answer}` : "Sai rồi! Chọn lại lần nữa.";
    } else {
      // If correct answers are hidden, client checks it locally
      isCorrect = true; // Fallback or mock
  ...
  // Submit Essay Draft (Max 3 drafts)
  const submitEssayDraft = (qId) => {
    const currentAns = sessionAnswers[qId];
    const draft = currentAns.draftText.trim();
    if (!draft || currentAns.locked) return;
    ...
    addToast(`Đã nộp lượt tự luận thứ ${updatedAttempts}/3!`, "success");
  };
  ...
  const getBubbleClass = (qId, type) => {
    ...
    if (type === "multiple_choice") {
      ...
      if (ans.locked) return "bg-green-500/15 border-green-600/50 text-green-400";
      if (ans.drafts.length > 0) {
        return ans.attempts >= 3 ? "bg-green-500/15 border-green-600/50 text-green-400" : "bg-amber-500/15 border-amber-600/50 text-amber-400";
      }
                ...
                {activeQuiz.questions[activeQuestionIndex].question_type === "multiple_choice" && (
                  <div className="flex flex-col gap-2.5">
                    ...
                    <div className="mt-4 flex items-center gap-4">
                      <button
                        onClick={() => checkMultipleChoiceAnswer(activeQuiz.questions[activeQuestionIndex].id)}
                        disabled={sessionAnswers[activeQuiz.questions[activeQuestionIndex].id]?.locked || !sessionAnswers[activeQuiz.questions[activeQuestionIndex].id]?.selectedOption}
                ...
                {activeQuiz.questions[activeQuestionIndex].question_type === "essay" && (
                  <div className="flex flex-col gap-4">
                    <textarea
                      ...
                      placeholder="Viết lời giải tự luận tại đây..."
                    />

                    <button
                      onClick={() => submitEssayDraft(activeQuiz.questions[activeQuestionIndex].id)}
                      ...
                      className="w-fit bg-brand-600 hover:bg-brand-500 disabled:bg-gray-800 disabled:text-gray-500 text-xs font-bold px-4 py-2.5 rounded-lg transition"
                    >
                      Nộp lượt bài làm này
                    </button>
                  </div>
                )}
              </div>
                ...
                <div className="grid grid-cols-5 gap-2">
                  {activeQuiz.questions.map((q, index) => (
                    <button
                      ...
                      } ${getBubbleClass(q.id, q.question_type)}`}
                    >
                      {index + 1}
                    </button>
                  ))}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {["gemini", "openai"].map((p) => (
                    <button
                      key={p}
                      onClick={() => setSettings((prev) => ({ ...prev, provider: p, model: p === "gemini" ? "gemini-2.5-flash" : "gpt-4o-mini" }))}
